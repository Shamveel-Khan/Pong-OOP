...
class paddle {
    ...
    public:
        paddle(int px,int py,int width,int height,Color color,int speed) {
            ...
            Image paddle = LoadImage("paddle2.png");
...
int main () {
    ...
    SetTargetFPS(60);
    Image neonbackground = LoadImage("neonbackground.png");
    ImageResize(&neonbackground,800,800);

    Texture2D img = LoadTextureFromImage(neonbackground);
    Image r = LoadImage("resumeButton.png");
    ImageResize(&r,200,200);
    
    Texture2D re = LoadTextureFromImage(r);
    ...
    paddle left(10,screenHeight/2 - 60,20,150,WHITE,7);
    paddle right(screenWidth-35,screenHeight/2 - 60,20,150,WHITE,7);
    Rectangle boundaries= {0,0,800,800};
    while(!WindowShouldClose()) {
        BeginDrawing();
        ClearBackground(BLACK);
        if(!toggleGear && !togglePause){
        ...
        if(!togglePause){
            DrawText(TextFormat("%i",scoreLeft),140,300,260,(Color){ 255, 172, 28,255});
            DrawText(TextFormat("%i",scoreRight),550,300,260,(Color){ 0, 191, 255, 255 });
            DrawLineEx({400,0},{400,800},5.0f,(Color){ 223, 255, 0,200 });
       ...
       if(showPause){
        ...
        if(toggleGear){
         ...
         if(togglePause){
            ...
            DrawTexture(re,300,230,WHITE);
            DrawText("PRESS SPACE TO CONTINUE",260,430,20,{255,191,0,200});
            showGear = false;
