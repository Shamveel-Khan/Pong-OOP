...
class paddle {
    ...
    public:
        paddle(int px,int py,int width,int height,Color color,int speed) {
            ...
            Image paddle = LoadImage("paddle.png");
...
int main () {
    ...
    InitWindow(800,800,"Under water");
    SetTargetFPS(60);
    Image underWaterBackground = LoadImage("underwaterbackground.png");
    ImageResize(&underWaterBackground,800,800);

    Texture2D img = LoadTextureFromImage(underWaterBackground);
    ...
    paddle left(10,screenHeight/2 - 60,30,150,WHITE,7);
    paddle right(screenWidth-35,screenHeight/2 - 60,30,150,WHITE,7);
    ...
    while(!WindowShouldClose()) {
        ...
        DrawText(TextFormat("%i",scoreLeft),140,300,260,(Color){ 120, 255, 255, 90 });
        DrawText(TextFormat("%i",scoreRight),550,300,260,(Color){ 120, 252, 255, 90 });
        DrawLineEx({400,0},{400,800},8.0f,(Color){ 150, 252, 255, 90 });
       ...
       if(showPause){
        ...
        if(toggleGear){
         ...
         if(togglePause){
            DrawText("Game Is Paused",200,370,50,RAYWHITE);
            DrawText("PRESS SPACE TO CONTINUE",244,430,20,RAYWHITE);
            showGear = false;
