...
int main () {
    ...
    InitWindow(800,800,"Forest And Wood");
    SetTargetFPS(60);
    Image forestBackground = LoadImage("forestbackground.png");
    ImageResize(&forestBackground,800,800);

    Texture2D img = LoadTextureFromImage(forestBackground);
    ...
    paddle right(screenWidth-45,screenHeight/2 - 60,40,150,WHITE,7);
    ...
    while (!WindowShouldClose())
    {
        ...
        DrawText(TextFormat("%i",scoreLeft),83,325,120,(Color){ 124, 252, 0, 255 });
        DrawText(TextFormat("%i",scoreRight),657,325,120,(Color){ 124, 252, 0, 255 });
        DrawLineEx({400,0},{400,800},8.0f,(Color){ 101, 67, 33, 255 });
       ...
       if(showPause){
        ...
        if(toggleGear){
         ...
         if(togglePause){
            DrawText("Game Is Paused",150,370,60,RAYWHITE);
            DrawText("PRESS SPACE TO CONTINUE",210,430,24,RAYWHITE);
            showGear = false;
